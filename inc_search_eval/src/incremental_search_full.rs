...
/// This has complexity of O(k) = O(min(k,p))
#[inline(always)]
...
/// This has complexity O(k+c^2) TODO: verify
...
pub fn incremental_search_full<const NC_LIMIT: usize, const NC_LIMIT_PLUS1: usize>(
    ...
    if n_c_max == 0 || NC_LIMIT == 0 {
        ...
        incremental_search_arq_full::<NC_LIMIT,NC_LIMIT_PLUS1>(channel_erasure_rate, target_erasure_rate, timestats, max_kp)
    }
}
...
pub fn incremental_search_arq_full<const NC_LIMIT: usize, const NC_LIMIT_PLUS1: usize>(
    channel_erasure_rate: f64,
    target_erasure_rate: f64,
    timestats: &TimeStats,
    max_kp: u16,
    ...
    // The greedy insert now is used as a "insert next packet"
    ...
    // Full rebuild.
    let rebuild_schedule = |k: u16,

                            schedule: &mut [u16; NC_LIMIT_PLUS1],
                            schedule_len: &mut usize,
                            schedule_max_len: usize,
                            schedule_duration: &mut Duration| {
        let target_p: u16 = schedule.iter().sum::<u16>() + 1u16;
        // Reset schedule to base state (only source packets)
        *schedule = [0; NC_LIMIT_PLUS1];
        *schedule_len = 0;
        // The duration of just the K source packets
        *schedule_duration = k1_delay + timestats.source_interval.mul_f64((k - 1) as f64);

        // Greedily insert all parity packets from scratch
        for _ in 0..target_p {
            greedy_add_parity(
                k,
                schedule,
